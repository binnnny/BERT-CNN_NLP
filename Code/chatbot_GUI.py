...
class SentimentAnalyzer:
    ...
    def get_sentiment(self, text):
        ...
        # Convert probabilities to labels
        sorted_probs_idx = sorted(range(len(probabilities)), key=lambda k: probabilities[k], reverse=True)
        labelss = self.labels[sorted_probs_idx[0]]  # Assuming labels is defined within the class
...
class ChatWindow(tk.Tk):
    ...
    def send_message(self):
        ...
        sentiment = self.sentiment_analyzer.get_sentiment(message)  # sentiment 분석
        self.sentiment_analyzer.create_chat(sentiment, text=message)  # Chat creation
